	...
	<nav class="closed panel-group">
		<div class="panel panel-default visible-xs-block">
			<div class="panel-heading">
				<a data-toggle="toggleClass" data-target="#sidebar" data-classes="opened closed"><i class="fa fa-fw fa-bars"></i></a>
			</div>
		</div>
		<div class="panel panel-default" v-for="line in directory">
			<div class="panel-heading">
				<a v-if="line.children" data-toggle="collapse" :data-target="`#${ line.code }`" data-parent="#sidebar" class="directory collapsed"><i v-if="line.icon" class="fa fa-fw" :style="`fa-${ line.icon }`"></i><i class="fa fa-fw first-word" v-else>{{ line.title.substring(0, 1) }}</i> <span>{{ line.title }}</span> <span class="caret"></span></a>
